...
import { RoomScanError } from '../types/RoomLayout';
 ...
 * ```
 */
export function useRoomScanner(options = {}) {
    ...
    useEffect(() => {
        if (!roomPlanEmitter)
            return;
        const progressSub = roomPlanEmitter.addListener('onScanProgress', (data) => {
            setProgress(data.progress);
            onProgress?.(data.progress);
        });
        const completeSub = roomPlanEmitter.addListener('onScanComplete', (layout) => {
            setIsScanning(false);
            setRoomLayout(layout);
            setProgress(1);
            onScanComplete?.(layout);
        });
        const errorSub = roomPlanEmitter.addListener('onScanError', (errorData) => {
    ...
    const startScan = useCallback(async () => {
        if (!isSupported) {
            const err = new RoomScanError('Room scanning not supported on this device', 'UNSUPPORTED');
            setError(err);
            onError?.(err);
            return;
        }
        ...
        try {
            ...
            const scanError = new RoomScanError(err.message || 'Scan failed', err.code || 'SCAN_ERROR', err);
            setIsScanning(false);
            setError(scanError);
            onError?.(scanError);
        }
